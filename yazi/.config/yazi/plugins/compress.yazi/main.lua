...
local is_password, is_encrypted, is_level, cmd_password, cmd_level, default_extension =
	false, false, false, "", "", "zip"
...
return {
	entry = function(_, job)
		...
		local archive_cmd, 
archive_args, archive_compress, archive_level_arg, archive_level_min, archive_level_max, archive_header_arg, archive_passwordable, archive_compress_args
		...
		if archive_level_arg ~= "" and is_level then
			...
			if
				output_level ~= ""
				and tonumber(output_level) ~= nil
				and tonumber(output_level) >= archive_level_min
				and tonumber(output_level) <= archive_level_max
			then
				cmd_level = type(archive_level_arg) == "table" and archive_level_arg[#archive_level_arg] .. output_level
					or archive_level_arg .. output_level
		...
		for filepath, filenames in pairs(path_fnames) do
			...
			if not archive_status or not archive_status.success then
				...
				if not cleanup_status then
					notify_error(
						string.format("Failed to clean up temporary directory %s, error: %s", temp_dir, cleanup_err),
						"error"
					)
				end
				return
		...
		local final_output_url, temp_url_processed =
			combine_url(output_dir, original_name), combine_url(temp_dir, original_name)
		...
		if not move_status then
			...
			notify_error(
				string.format("Failed to move %s to %s, error: %s", temp_url_processed, final_output_url, move_err),
				"error"
			)
			local cleanup_status, cleanup_err = fs.remove("dir_all", Url(temp_dir))
			if not cleanup_status then
